...
export default function Dashboard() {
  ...
  const fetchCases = async () => {
    try {
      const res = await fetch('https://convey-ai-production-be43.up.railway.app/cases')
  ...
  const createCase = async () => {
    ...
    setCreating(true)
    try {
      const res = await fetch(`https://convey-ai-production-be43.up.railway.app/cases?title_number=${newTitleNumber}`, {
        method: 'POST'
      })
      const data = await res.json()
      if (data.success) {
