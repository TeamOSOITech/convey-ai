...
def extract_date(full_text: str, filename: str, is_oce: bool = False) -> str:
    # Only need the top of the doc for the date
    context = full_text[:3000]

    if is_oce:
        prompt = f"""You are reading an Official Copy of Register of Title.
TASK: Find the search date ("This official copy shows the entries on the register of title on [DATE] at [TIME]").
Return exactly the date and time string. Do NOT return the "Edition date".
If not found, return [NOT FOUND].
DOCUMENT: {context}"""
    else:
        prompt = f"""You are reading a UK legal deed: {filename}
TASK: Find the date this document was executed/signed.
Look for "dated [date]", "made on [date]", etc. Return ONLY the date string.
If not found, return [NOT FOUND].
DOCUMENT: {context}"""
...
def generate_title_report(title_number: str, selected_filenames: list) -> dict:
    ...
    for filename in selected_filenames:
        ...
        if not is_oce:
            fields = extract_legal_fields(full_text, filename)
            doc_result.update(fields)
